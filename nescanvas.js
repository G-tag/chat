...
var CLEANUP_SPEED = 15000;  //Call cleanup function after this many milliseconds
var DRAWING = false;
...
$(document).ready(function() {
    ...
    //Post browser notice
    //alert("nesCanvas works best in Firefox 3.5 or higher, but will also run OK in Chrome / Safari / Opera.");
...
function login() {
    ...
    //validate
    sel_NICK = $("#nick_sel").val().replace(/\W/g , '');
    ...
    else if(sel_NICK.length > 25) alert("Nicks are limited to 25 characters for now");
...
//	DRAWING = true;
	...
	for(var i = 0; i < SPRITES.length; i++) {
		...
		ctx.drawImage( SPRITES[i].getCurrentFrame() , SPRITES[i].x , SPRITES[i].y );
		//Write nickname
		ctx.shadowOffsetX = 2;
		ctx.shadowOffsetY = 2;
		//ctx.fillText( SPRITES[i].name , SPRITES[i].width/2, SPRITES[i].height + 25 );
		ctx.fillText( SPRITES[i].name , SPRITES[i].x + SPRITES[i].width/2, SPRITES[i].y + SPRITES[i].height + 25 );
...
function refreshCallback(jsResponse) {
	...
	if(numMsgs  > 0) {
		...
		if(which != -1) {
		    ...
		    var cleanName = jsr.Messages[i].name.replace(/</g , '&lt;');
		    cleanName = cleanName.replace(/>/ , '&gt;');
		    var cleanMsg = jsr.Messages[i].msg.replace(/</g , '&lt;');
		    cleanMsg = cleanMsg .replace(/>/ , '&gt;');
		    writeToConsole("<span class=\"chat\">" + cleanName + ": "+ cleanMsg + "</span>");
		}
	    }
	}
    }
...
function errorOut(x, s, e) {
    $("#console").prepend("<span class=\"error\">" + x.statusText + " *** " + e + "</span><br />");
}
